...
if ( $pid == 0 ) { # child
  ...
  # Write key via pipe to mosh-client.  Do this by forking a child to avoid
  # potential deadlock (unlikely, but standards do not prohibit it).
  pipe READER, WRITER;
  my $key_file_no = fileno READER;
  defined( my $pid = fork ) or die "$0: fork: $!\n";
  if ( $pid == 0 ) {
    # Double fork to avoid zombies.
    $SIG{CHLD} = "IGNORE";
    defined( my $pid = fork ) or die "$0: fork 2: $!\n";
    _exit 0 if ( $pid );
    ( print WRITER "MOSH_KEY=" . $key . "\n" and close WRITER ) or die "$0: write from child: $!\n";
    _exit 0;
  }
  (waitpid($pid, 0) == $pid) or die "$0: waitpid: $!\n";
  close WRITER;

  # Clear close-on-exec flag
  use Fcntl qw( F_GETFD F_SETFD FD_CLOEXEC );
  my $flags = fcntl( READER, F_GETFD, 0 ) or die "Can't get flags: $!\n";
  fcntl( READER, F_SETFD, $flags & ~FD_CLOEXEC ) or die "Can't set flags: $!\n";
  ...
  push @client_av, ( $ip, $port, $key_file_no );
