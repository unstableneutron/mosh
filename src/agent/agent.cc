...
ProxyAgent::ProxyAgent( bool is_server, bool dummy )
  : comm( NULL ), oob_ctl_ptr( NULL ), server( is_server ), ok( false ), l_sock( -1 ), l_dir( "" ), l_path( "" ),
    cnt( 0 ), agent_sessions()
{
#ifdef SUPPORT_AGENT_FORWARDING
  if ( dummy ) {
    return;
...
AgentConnection::AgentConnection( int sock, uint64_t id, ProxyAgent* s_agent_ptr )
  : s_in_read_set( false ), s_sock( sock ), s_id( id ), idle_start( Network::timestamp() ), packet_buf( "" ),
    packet_len( 0 ), agent_ptr( s_agent_ptr )
{
...
AgentConnection::AgentConnection( std::string agent_path, uint64_t id, ProxyAgent* s_agent_ptr )
  : s_in_read_set( false ), s_sock( -1 ), s_id( id ), idle_start( Network::timestamp() ), packet_buf( "" ),
    packet_len( 0 ), agent_ptr( s_agent_ptr )
{
#ifdef SUPPORT_AGENT_FORWARDING
  int sock = socket( AF_UNIX, SOCK_STREAM, 0 );
