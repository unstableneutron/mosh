...
OutOfBand::OutOfBand()
  : comms(), datagram_instruction_out(), reliable_instruction_out_sent(), reliable_instruction_out_unsent(),
    seq_num_out( 0 ), ack_num_out( 0 )
{
  seq_num_out = 0;
  ack_num_out = 0;
}
...
{
  return ( !( datagram_instruction_out.empty() && reliable_instruction_out_sent.empty()
              && reliable_instruction_out_unsent.empty() ) );
}
...
OutOfBandCommunicator::OutOfBandCommunicator( OutOfBandMode oob_mode,
                                              string oob_name,
                                              OutOfBand* oob_ctl,
                                              OutOfBandPlugin* plugin )
  : mode( oob_mode ), name( oob_name ), stream_buf( "" ), datagram_queue(), plugin_ptr( plugin ),
    oob_ctl_ptr( oob_ctl )
{
  /* */
}
